...
The notebook ``d4/01_latent_graph_analysis.ipynb`` is
...
NOTEBOOK = _ROOT / "d4/01_latent_graph_analysis.ipynb"
...
def main() -> None:
    ...
    parser.add_argument("--output-dir", type=Path,
                        default=_ROOT / "latex/d4/figures")
    ...
    try:
        ...
        produced = []
        # The figure, plus every CSV the notebook writes. Naming the CSVs
        # individually meant a newly saved one silently vanished with the
        # scratch directory, so the glob keeps the copy list honest.
        wanted = ["d4_latent_graph_analysis.pdf", "d4_latent_graph_analysis.png"]
        wanted += sorted(path.name for path in work.glob("d4_*.csv"))
        for name in wanted:
